...
function CheckoutForm() {
  ...
  const calculateTotals = () => {
    const subtotal = cart.reduce((sum, item) => 
      sum + (item.selectedPrice || item.price) * item.quantity, 0
    );
    return {
      subtotal,
      ...
      <div className="payment-header">
        ...
        <div className="order-summary">
          ...
          <div className="items-list">
            {cart.map((item) => (
              <div key={`${item.id || item._id}-${item.size || 'regular'}`} className="order-item">
                ...
                <div className="item-price">
                  ...
                  <span className="price">£{((item.selectedPrice || item.price) * item.quantity).toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="delivery-info">
          ...
          {orderDetails?.customerInfo && Object.entries(orderDetails.customerInfo).map(([key, value]) => (
            key !== 'email' && key !== 'specialInstructions' && (
